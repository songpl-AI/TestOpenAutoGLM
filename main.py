...
Environment Variables:
    PHONE_AGENT_PROVIDER: API provider (local/zhipu, default: local)
    PHONE_AGENT_BASE_URL: Model API base URL (default: http://localhost:8000/v1)
    PHONE_AGENT_MODEL: Model name (default: autoglm-phone-9b or glm-4v-plus)
    PHONE_AGENT_API_KEY: API key for cloud providers
    ZHIPU_API_KEY: 智谱 AI API key (alias for PHONE_AGENT_API_KEY)
    PHONE_AGENT_MAX_STEPS: Maximum steps per task (default: 100)
    PHONE_AGENT_DEVICE_ID: ADB device ID for multi-device setups
    PHONE_AGENT_LANG: Language (cn/en, default: cn)
"""
...
def check_model_api(base_url: str, model_name: str, provider: str = "local", api_key: str = "EMPTY") -> bool:
    """
    ...
    try:
        ...
        # 对于智谱 AI，需要提供有效的 API key
        if provider == "zhipu":
            if not api_key or api_key == "EMPTY":
                print("❌ FAILED")
                print("   Error: 智谱 AI requires an API key")
                print("   Solution:")
                print("     1. Get API key from https://open.bigmodel.cn")
                print("     2. Set environment variable: export ZHIPU_API_KEY='your-key'")
                print("     3. Or use --api-key option")
                return False
        ...
        if model_name in available_models:
            ...
            # 对于智谱 AI，某些模型可能不在列表中但仍然可用
            if provider == "zhipu" and model_name in ["glm-4.6v", "glm-4.5v", "glm-4v-plus", "glm-4v", "glm-4"]:
                print("⚠️  OK (model should be available)")
            else:
                print("❌ FAILED")
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
    # Run with local vLLM (default)
    python main.py

    # Use 智谱 AI API
    python main.py --provider zhipu --api-key your-api-key --model glm-4v-plus
    ...
    parser.add_argument(
        ...
        type=str,
        choices=["local", "zhipu"],
        default=os.getenv("PHONE_AGENT_PROVIDER", "local"),
        help="API provider (local: vLLM/SGLang, zhipu: 智谱 AI)",
    )
    ...
    parser.add_argument(
        "--api-key",
        type=str,
        default=os.getenv("ZHIPU_API_KEY") or os.getenv("PHONE_AGENT_API_KEY", "EMPTY"),
        help="API key (for zhipu provider)",
    )
...
def main():
    ...
    if args.provider == "zhipu":
        # 智谱 AI 使用固定的 base_url
        if args.base_url == "http://localhost:8000/v1":  # 默认值
            args.base_url = "https://open.bigmodel.cn/api/paas/v4"
        
        # 智谱 AI 使用固定的模型
        if args.model == "autoglm-phone-9b":  # 默认值
            args.model = "glm-4.6v"

    # Check model API connectivity and model availability
    if not check_model_api(args.base_url, args.model, args.provider, args.api_key):
        sys.exit(1)
    ...
    if args.provider == "zhipu":
        # 使用智谱 AI
        from phone_agent.model import ZhipuAPIConfig
        
        model_config = ZhipuAPIConfig(
            api_key=args.api_key,
            base_url=args.base_url,
            model_name=args.model,
        )
    else:
        # 使用本地 vLLM 或其他 OpenAI 兼容 API
