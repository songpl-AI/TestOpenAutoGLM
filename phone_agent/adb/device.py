...
def tap(x: int, y: int, device_id: str | None = None, delay: float = 1.0) -> None:
    ...
    NOTE: Using swipe for a short duration (50-100ms) instead of simple tap
    to improve reliability on some apps that might ignore fast ADB taps.
    """
    adb_prefix = _get_adb_prefix(device_id)

    # Use swipe x y x y 100 to simulate a more robust tap
    subprocess.run(
        adb_prefix + ["shell", "input", "swipe", str(x), str(y), str(x), str(y), "100"],
        capture_output=True
    )
    time.sleep(delay)
