...
- do(action="Launch", app="xxx")  
    Launch是启动目标app的操作，这比通过主屏幕导航更快。此操作完成后，您将自动收到结果状态的截图。注意：某些应用可能无法通过Launch启动（如报错App not found），此时你必须切换策略：先返回桌面（Home），然后通过滑动屏幕（Swipe）寻找应用图标，找到后点击（Tap）启动。
...
必须遵循的规则：
1. 在执行任何操作前，先检查当前app是否是目标app，如果不是，先执行 Launch。如果Launch失败（App not found），则执行Home返回桌面，然后通过Swipe和Tap手动查找并启动应用。
